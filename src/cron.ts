...
export const CronJob = async (env: Env) => {
	...
	console.log(diff);
	if (diff > 0.03) {
		await LineNotify(env, `MAX Sell Price ${maxSellPrice} is higher than HOYA Buy Price ${hoyaBuyPrice} more than 0.03\nPrice diff: ${diff}`);
		// } else {
		//   await LineNotify(env, `MAX Sell Price ${maxSellPrice} is not higher than HOYA Buy Price ${hoyaBuyPrice} less than 0.03\nPrice diff: ${diff}`)
	}
